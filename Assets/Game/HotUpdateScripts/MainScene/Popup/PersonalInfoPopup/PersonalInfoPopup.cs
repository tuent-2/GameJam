...
public class PersonalInfoPopup : BasePopup<PersonalInfoPopup>
{
    ...
    public void UpdatePopupVisual()
    {
        ...
        _tmpGame1Info.text = $"Balance tỉ lệ thắng : <color=green>{game1Data.totalWin}</color>/<color=white>{(game1Data.totalWin/game1Data.totalMatch)*100}</color> (<color=yellow>x</color>%)";

        var game2Data = data.ThanhTuus[1];
        _tmpGame2Info.text = $"Discrepancy tỉ lệ thắng : <color=green>{game2Data.totalWin}</color>/<color=white>{(game2Data.totalWin/game2Data.totalMatch)*100}</color> (<color=yellow>x</color>%)";
