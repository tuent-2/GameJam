    ...
    public class EnhancedScroller : MonoBehaviour
    {
        ...
        public float LinearVelocity
        {
            get
            {
                ...
                return (scrollDirection == ScrollDirectionEnum.Vertical
                    ? _scrollRect.velocity.y
        ...
        {
            get { return _activeCellViewsStartIndex; }
        }
        ...
        {
            get { return _activeCellViewsEndIndex; }
        }
        ...
        {
            get { return _activeCellViewsStartIndex % NumberOfCells; }
        }
        ...
        {
            get { return _activeCellViewsEndIndex % NumberOfCells; }
        }
        ...
        public void ReloadData(float scrollPositionFactor = 0)
        {
            ...
            _scrollPosition = Mathf.Clamp(scrollPositionFactor * ScrollSize, 0,
                GetScrollPositionForCellViewIndex(_cellViewSizeArray.Count - 1, CellViewPositionEnum.Before));
            if (scrollDirection == ScrollDirectionEnum.Vertical)
            {
                // set the vertical position
        ...
        )
        {
            ...
            else
            {
                ...
                newScrollPosition = Mathf.Clamp(newScrollPosition - (useSpacing ? spacing : 0), 0,
                    GetScrollPositionForCellViewIndex(_cellViewSizeArray.Count - 1, CellViewPositionEnum.Before));
            }
            ...
            else
            {
                if (cellViewIndex < _cellViewOffsetArray.Count)
                {
                    ...
                    else
                    {
                        ...
                        return _cellViewOffsetArray[cellViewIndex] + (scrollDirection == ScrollDirectionEnum.Vertical
                            ? padding.top
                            : padding.left);
                    }
                }
        ...
        {
            return GetScrollPositionForCellViewIndex(loop ? _delegate.GetNumberOfCells(this) + dataIndex : dataIndex,
                insertPosition);
        }
        ...
        public void _Resize(bool keepPosition)
        {
            ...
            _container.sizeDelta = scrollDirection == ScrollDirectionEnum.Vertical
            ...
            if (loop)
            {
                _loopFirstScrollPosition =
                    GetScrollPositionForCellViewIndex(_loopFirstCellIndex, CellViewPositionEnum.Before) +
                    (spacing * 0.5f);
                _loopLastScrollPosition =
                    GetScrollPositionForCellViewIndex(_loopLastCellIndex, CellViewPositionEnum.After) - ScrollRectSize +
                    (spacing * 0.5f);
        ...
        private void _SetPadders()
        {
            ...
            var firstSize = _cellViewOffsetArray[_activeCellViewsStartIndex] -
                            _cellViewSizeArray[_activeCellViewsStartIndex];
        ...
        private void _CalculateCurrentActiveCellRange(out int startIndex, out int endIndex)
        {
            ...
            var endPosition = _scrollPosition + (scrollDirection == ScrollDirectionEnum.Vertical
                ? _scrollRectTransform.rect.height
                : _scrollRectTransform.rect.width);
        ...
        private void _ScrollRect_OnValueChanged(Vector2 val)
        {
            ...
            //_refreshActive = true;
            _scrollPosition = Mathf.Clamp(_scrollPosition, 0,
                GetScrollPositionForCellViewIndex(_cellViewSizeArray.Count - 1, CellViewPositionEnum.Before));
